"""Functional API for data split management.

This module provides simple functions for loading and saving data splits,
replacing the class-based DataSplitRepository for simpler use cases.

Example:
    ```python
    from aee.infrastructure.storage.splits import load_split, load_all_splits
    ...
    # Load all splits
    all_splits = load_all_splits(Path("data/splits/nanozymes.json"))
    ```
"""
...
def save_splits(
    splits: Dict[str, List[str] | Set[str]],
    output_path: Path,
) -> Path:
...
# Keep DataSplitRepository for backward compatibility
